...
REFRESH_INTERVAL = 0.1  # Alle 100ms refreshen (langsamer = weniger Flickering)
...
arduino = None
stop_refresh = False
arduino_lock = threading.Lock()  # Verhindert gleichzeitiges Schreiben
...
def send_to_arduino(ser, led_index, velocity, show_print=True):
    """Sendet LED-Befehl an Arduino (Thread-safe)"""
    with arduino_lock:
        ...
        ser.flush()  # Wichtig: Sicherstellen dass Daten gesendet werden

        if show_print:
            print(f"  → Arduino: LED {led_index}, Helligkeit {brightness}")
...
def refresh_worker():
    """Background-Thread der gehaltene Tasten refresht"""
    global active_notes, arduino, stop_refresh

    while not stop_refresh:
        if active_notes and arduino:
            # Alle aktiven Notes nochmal senden (ohne Print)
            for note, (led_index, velocity) in list(active_notes.items()):
                send_to_arduino(arduino, led_index, velocity, show_print=False)
...
def main():
    ...
    # MIDI-Ports anzeigen
    print("=" * 50)
    print("MIDI → Arduino LED Controller (Stable)")
    print("=" * 50)
    ...
    # Refresh-Thread starten
    refresh_thread = threading.Thread(target=refresh_worker, daemon=True)
    refresh_thread.start()
    print("  ✓ Refresh-Thread gestartet (100ms Intervall)")
    ...
    try:
        with mido.open_input(midi_port_name) as inport:
            # BLOCKING read - wartet auf MIDI messages (zuverlässig!)
            for msg in inport:
