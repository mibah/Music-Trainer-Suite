...
class UI:
    ...
    def draw_piano(self, current_notes):
        ...
        for i, note in enumerate(WHITE_NOTES):
            ...
            color = HIGHLIGHT_COLOR if note in current_notes else WHITE
        ...
        for note, index in BLACK_NOTES.items():
            if WHITE_NOTES[index] not in white_key_positions:
                continue
            x = white_key_positions[WHITE_NOTES[index]] + int(KEY_WIDTH * 0.75)
            color = HIGHLIGHT_COLOR if note in current_notes else BLACK
